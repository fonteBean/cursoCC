...
public class Inss {
  ...
  public static class Fila {
    ...
    public String remover() throws Exception {
      ...
      String nome = inicio.prox.getNome();
      Game tmp = inicio;
      inicio = inicio.prox;
      tmp.prox = null;
      tmp = null;
    ...
    public Game buscaGame(int id) {
      for (Game g = inicio.prox; g != null; g = g.prox) {
        if (g.getId() == id) {
          return g;
        }
      }
  ...
  public static void menu(Fila baseCsv, Fila fila, String entrada) throws Exception {
  ...
  public static Game buscaGame(Fila jogos, int id) {
    Game g = jogos.buscaGame(id);
    return g != null ? g.clone() : null;
  }

  public static Fila criaSubFila(Fila jogos, Scanner scanf) {
  ...
  public static void main(String[] args) {
    ...
    Fila subFila = criaSubFila(jogos, scanf);

    try {
      int n = Integer.parseInt(scanf.nextLine().trim());
      for (int i = 0; i < n; i++) {
        menu(jogos, subFila, scanf.nextLine());
      }
      subFila.mostrar();
    } catch (Exception e) {
      System.out.println(e.getMessage());
