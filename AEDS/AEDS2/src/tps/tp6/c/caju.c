...
void inserirNoInicio(Lista *lista, Game *g)
{
    g->prox = lista->p;
    lista->p = g;
    if (lista->u == NULL)
...
void inserir(Lista *lista, Game *g, int pos)
{
    if (pos == 0)
    {
...
void inserirNoFim(Lista *lista, Game *g)
{
    g->prox = NULL;
    if (lista->p == NULL)
    {
...
char *removerDoInicio(Lista *lista)
{
    if (!lista->p)
        return NULL;
...
void menu(Lista *baseCsv, Lista *lista, const char *entrada)
{
    ...
    {
        char *nome = removerDoInicio(lista);
        if (nome)
        {
            printf("(R) %s\n", nome);
            free(nome);
...
Lista *criaSubLista(Lista *jogos)
{
    Lista *subLista = novaLista();
    char linha[TAMFIELD];
...
int main()
{
    ...
    Lista *subLista = criaSubLista(jogos);
