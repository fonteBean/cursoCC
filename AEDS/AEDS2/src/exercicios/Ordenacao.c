    ...
    for (int i = 0;; i++) {
        ...
        if (fim || (c == ',' && !entreAspas)) {
            ...
            switch (idx) {
                ...
                case 2: j->releaseDate = dataFudida(field); break;
                case 3: {
...
void escreveLog(double tempo) {
    FILE *arquivo = fopen("889080_selecao.txt", "w"); 
    if (arquivo == NULL) {
...
void selection(Game jogos[], int n) {
    for (int i = 0; i < n - 1; i++) {
        int menor = i;
        for (int j = i + 1; j < n; j++) {
            comparacoes++;
            if (strcmp(jogos[j].name, jogos[menor].name) < 0) { 
                menor = j;
            }
        }
        if (menor != i) {
